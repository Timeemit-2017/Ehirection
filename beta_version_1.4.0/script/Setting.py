...
class Setting():
    ...
    def load_settings(self):
        ...
        for key in SettingVar.keys:
            self.settings.append(KeySet(key_set_titles[i], key, False, self.video_size))
            i += 1
...
class Set(object):
    ...
    def draw(self, canvas, position, mousePos=(0, 0)):
        self.pos = position
...
class KeySet(Set):
    ...
    def draw(self, canvas, position, mousePos):
...
class KeySetButton():
    ...
    def draw(self, canvas, pos, mousePos):
        self.x = pos[0]
        self.y = pos[1]
