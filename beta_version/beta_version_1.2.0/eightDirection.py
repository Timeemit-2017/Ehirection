...
@author: Time_emit
'''
import pygame,random,time,sys,os,pygame.gfxdraw
...
def handleEvent():
    ...
    for event in events:
        ...
        if event.type == VIDEORESIZE:
            ...
            for item in GameVar.lobbyObjects:
                if item.item and (event.type == MOUSEBUTTONDOWN or event.type == MOUSEMOTION) and item.checkRange((event.pos[0] - (WIDTH - 62 * 8) / 2), (event.pos[1] - (HEIGHT - 62 * 8) / 2), 1, 1):
                    if event.type == MOUSEBUTTONDOWN and event.button == 1:
...
class GameVar():
    ...
    home = Home()
    #使用字典存储游戏进程
    STATES={"HOME_0":0,"HOME_1":1,"SONGS_CHOOSE":2,"SONGS_CHOOSE_2":3,"START":4,"ITEM":5,"RUNNING":6,"GAME_OVER":7,"BOX":8,"BOX_GET":9,"LOBBY":10}
    States = []
    for state in STATES:
        States.append(state)
...
def hall_main():
    ...
    for item in GameVar.lobbyObjects:
        ...
        if item.item:
            item.draw_highlight(LobbyVar.canvas)
