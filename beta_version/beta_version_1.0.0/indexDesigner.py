...
# 初始化pygame环境
pygame.init()
pygame.mixer.music.load("songs/foundation_pack/solar_system.mp3")
# 创建一个长宽分别为480/650窗口
...
def control():
    ...
    if GameVar.states==GameVar.STATES["MAIN_0"]:
        ...
        song = easygui.choicebox(msg="请选择你要打开的歌曲", title="打开歌曲", choices=songs)
        song=eval(song)
        index=1
        while index<len(song):
            if song[index]==1:
                img=GameVar.arrowImgs[0]
            elif song[index]==2:
                img = GameVar.arrowImgs[1]
            elif song[index] == 3:
                img = GameVar.arrowImgs[2]
            elif song[index] == 4:
                img = GameVar.arrowImgs[3]
            GameVar.arrowList.append(Arrow(img,song[index+1], False, song[index]))
            index+=2
